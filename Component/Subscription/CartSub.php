...
* @author webnmobile
*/ 
class CartSub extends \Component\Cart\Cart
{
    ...
    public function setMemberCouponApply($cartSno, $memberCouponNo)
    {
        ...
        if ($memberCouponNo) {
            ...
            if ($memberCouponUsable) {
				...
				//$coupon->setMemberCouponState($memberCouponNo, 'sub', false, $cartSno);//기존 sub로 되어 있었음-이유알수 없음
            } else {
    ...
    protected function getGoodsDcData($goodsDiscountFl, $goodsDiscount, $goodsDiscountUnit, $goodsCnt, $goodsPrice, $fixedGoodsDiscount = null, $goodsDiscountGroup = null, $goodsDiscountGroupMemberInfo = null)
    {
		...
		if($subscription==1){
			...
			$subCfg = $subObj->getCfg($ArrGoodsNo);

			//gd_debug($goodsDiscount);
                ...
                default:
                    ...
                    if ($goodsDiscountUnit === 'percent') {
						...
						//gd_debug($tmp);
							...
							//gd_debug($tmp['discountByPrice'] * $discountPercent);
                        // 상품할인금액
                        $goodsDcPrice = gd_number_figure($tmp['discountByPrice'] * $discountPercent, $tmp['trunc']['unitPrecision'], $tmp['trunc']['unitRound']) * $goodsCnt;
                    } else {
                        // 상품금액보다 상품할인금액이 클 경우 상품금액으로 변경
	...
	*/
	public function setMergeGuestCart($memNo = null)
    {
		...
		if($memNo) {
			// 정기결제 장바구니
			$cartIdx = $this->setCartGoodsCnt($memNo, $cartIdx);
			$cartInfo = $this->getCartGoodsData($cartIdx, null, null, false, true);
			$cartSno = $this->cartSno;

			if($cartSno) {
				foreach($cartSno as $key => $val) {
					// 장바구니 쿠폰 초기화
					$this->setMemberCouponDelete($val);
				}
			}
		}
		// 2024-12-11 wg-eric 사용중인 쿠폰 취소 - end

		//2024-10-04 wg-brown 기존에 있던 상품삭제
		$sql="delete from wm_subCart where memNo=".$memNo." and directCart='y'";
		$this->db->query($sql);
		...
		$this->db->set_update_db("wm_subCart", $param, "siteKey = ? AND memNo = 0", $bind);
	}
	...
	*/
	public function addTogetherGoods($post = [])
	{
		if (!$post['tgGoodsNo'])
			return false;
		...
		$this->db->set_delete_db("wm_subCart", "siteKey = ? AND memNo = ? AND isTogetherGoods = 1", ["si", $siteKey, $memNo]);
		
		$cartSnos = [];
		foreach ($post['tgGoodsNo'] as $k => $goodsNo) {
			$sql = "SELECT sno FROM " . DB_GOODS_OPTION . " AS go 
					INNER JOIN " . DB_GOODS . " AS g ON go.goodsNo = g.goodsNo
				WHERE go.goodsNo = ? AND go.optionSellFl='y' AND g.delFl='n'  AND g.goodsSellFl='y' ORDER BY go.optionNo";
			...
			$arrData = [
				...
				'goodsCnt' => [gd_isset($post['tgGoodsCnt'][$k], 1)],
				'isTogetherGoods' => [1],
			];
			
			$d = $this->saveInfoCart($arrData);
			if ($d && $d[0]) {
				$cartSnos[] = $d[0];
			}
		}
	...
	public function getCartGoodsData($cartIdx = null, $address = null, $tmpOrderNo = null, $isAddGoodsDivision = false, $isCouponCheck = false, $postValue = [], $setGoodsCnt = [], $setAddGoodsCnt = [], $setDeliveryMethodFl = [], $setDeliveryCollectFl = [], $deliveryBasicInfoFl = false)
    {
        ...
        if (Cookie::has('isDirectCart')
            ...
            && (Request::getFileUri() != 'order_ps.php' || (Request::getFileUri() == 'order_ps.php' && in_array(Request::post()->get('mode'), ['set_recalculation']) === true))
            && $this->isWrite !== true
        ) {
