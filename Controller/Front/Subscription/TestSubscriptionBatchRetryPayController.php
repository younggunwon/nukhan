...
class TestSubscriptionBatchRetryPayController extends \Controller\Front\Controller
{
    private $db="";
    ...
    public function index()
    {
		...
		//$sql  = "
			...
			//order by idx ASC
		//";
        //$rows = $this->db->query_fetch($sql,['iss', 11794, 'first','middle']);
						...
						if($rcount<$retry_count){
							if($bool!=1){
								if($subCfg['rFailSmsUse']=='y'){
									...
									$payDate['payDate']   = date("Y.m.d", $nextROW[0]['deliveryStamp'] - (60 * 60 * 24 * $subCfg['payDayBeforeDelivery']));     
