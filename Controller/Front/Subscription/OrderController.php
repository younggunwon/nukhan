...
class OrderController extends \Controller\Front\Controller
{
	public function pre()
	{
		...
		if ($server['REMOTE_ADDR'] != '121.124.86.129' && $server['REMOTE_ADDR'] != '121.173.70.193') {
			return $this->js("window.history.back();");
		}
		*/


		///if(\Request::getRemoteAddress()=="112.146.205.110"){

		//}else{
			//카드결제 심사용시작
		//	$this->getView()->setPageName('subscription/orderInicis.php');
			//카드결제 심사용종료
		//}
    ...
    public function index()
    {
        try {
			...
			/* 함께 구매하기 처리 S */
			$tgCartSno = $cart->addTogetherGoods($in);
			if ($tgCartSno) {
				$cartIdx = array_merge($cartIdx, $tgCartSno);
				$this->setData('cartIdx', $cartIdx);
			}
			/* 함께 구매하기 처리 E */
                                ...
                                if (empty($_cartInfo['memberCouponNo']) === false) {
                                    ...
                                    foreach ($tmpApplyMemberCouponList as $tmpApplyMemberCouponNo) {
                                        if (array_key_exists($tmpApplyMemberCouponNo, $checkDuplMemberCouponNo) === true) {
                                            throw new AlertRedirectException(__('이미 사용중인 쿠폰이 적용되어 있습니다.'), null, null, '../subscription/cart.php');
                                        }
                                        $checkDuplMemberCouponNo[$tmpApplyMemberCouponNo] = $tmpApplyMemberCouponNo;
                                    }
                                }
								$this->setData('orderGoodsNm', $_cartInfo['goodsNm']);
                            }
                        }
                    }
                }
	...
	{
		$this->addCss(["order/order.css", "musign/order.css"]);
		...
		if(empty($in['deliveryPeriod'])){
			...
			$sql="select * from wm_subCart where 1=1 ".$where." and directCart='y'";
			$row = $db->query_fetch($sql);

			$in['deliveryEa']=$row[0]['deliveryEa'];
			$in['deliveryPeriod']=$row[0]['sPeriod'];
		}
		$deliveryEa = gd_isset($in['deliveryEa'], $subConf['deliveryEa'][0]);
		$deliveryPeriod = gd_isset($in['deliveryPeriod'], $subConf['deliveryCycle'][0]);
		$this->setData("deliveryEa", $deliveryEa);
		$this->setData("deliveryPeriod", $deliveryPeriod);
		$this->setData("autoExtend", $in['autoExtend']?1:0);
		...
		$this->setData("page","order");
		$member = Session::get("member");
        $this->setData("member", $member);

		//2024-09-24 wg-brown 정기결제 약관동의 가져오기
		$subscription = App::load(\Component\Subscription\Subscription::class);
		$conf = $subscription->getCfg();
		$this->setData('terms', $conf['terms']);
		$this->setData('cardTerms', $conf['cardTerms']);
		//2024-03-06 wg-brown 정기결제 약관동의 가져오기
