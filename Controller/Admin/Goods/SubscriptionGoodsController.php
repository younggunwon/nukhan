...
class SubscriptionGoodsController extends \Controller\Admin\Controller
{
	...
	{
		try {
			$this->callMenu("goods", "subscription", "goods");
			// --- 모듈 호출
			...
			if ($getData['data']) {
				foreach ($getData['data'] as $k => $v) {
					...
					$cfg = $subscription->getCfg([$v['goodsNo']]);
					$discount = $cfg['deliveryEaDiscount'];
					rsort($discount, SORT_NUMERIC);
					$row['discount'] = gd_isset($discount[0], 0);
					$v = array_merge($v, $row);
					$getData['data'][$k] = $v;
				}
			} // endif
